...
enum Command {
    ...
    /// Generate a portable plugin or bind an existing plugin to this workspace.
    Plugin {
    ...
    Mcp {
        ...
        #[arg(long, required_unless_present = "binding", conflicts_with = "binding")]
        workspace: Option<String>,
        /// Local binding generated by `plugin create`.
        #[arg(long, conflicts_with_all = ["database", "workspace", "codegraph", "codegraph_binary"])]
        ...
        codegraph: bool,
        /// Explicit `CodeGraph` executable path.
        #[arg(long, requires = "codegraph", conflicts_with = "binding")]
        codegraph_binary: Option<PathBuf>,
    },
}
...
#[derive(Debug, Subcommand)]
enum PluginCommand {
    /// Create a complete plugin or bind an existing portable plugin.
    Create {
        /// New, already-identical, or existing portable plugin directory.
        #[arg(long)]
        output: PathBuf,
        /// Existing MCP server entry; enables existing-plugin mode with `routing-skill`.
        #[arg(long, requires = "routing_skill")]
        mcp_server_name: Option<String>,
        /// Existing routing skill; enables existing-plugin mode with `mcp-server-name`.
        #[arg(long, requires = "mcp_server_name")]
        routing_skill: Option<String>,
        /// Workspace manifest path.
        #[arg(long, default_value = "code-system-graph.yaml")]
        config: PathBuf,
        /// Existing graph database path.
        #[arg(long, default_value = ".code-system-graph/code-system-graph.db")]
        database: PathBuf,
        /// Enable read-only CodeGraph-backed MCP tools.
        #[arg(long)]
        codegraph: bool,
        /// Explicit `CodeGraph` executable stored only in the ignored local binding.
        #[arg(long, requires = "codegraph")]
        codegraph_binary: Option<PathBuf>,
        /// Replace an older owned local receipt and binding in existing-plugin mode.
        #[arg(long, requires = "mcp_server_name")]
        replace_generated: bool,
    },
    /// Remove one integration previously managed by `plugin create`.
    Uninstall {
        /// Existing portable plugin directory.
        #[arg(long)]
        output: PathBuf,
...
async fn main() {
    if let Err(error) = run().await {
        let exit_code = if let Some(application) = error.downcast_ref::<ApplicationError>() {
            application_exit_code(application)
        } else if let Some(plugin) = error.downcast_ref::<AgentPluginError>() {
            agent_plugin_exit_code(plugin)
        } else {
            ExitCode::Internal
        };
        eprintln!("{error:#}");
    ...
    match cli.command {
        ...
        Command::Plugin { command } => match command {
            PluginCommand::Create {
                ...
                let target = match (mcp_server_name, routing_skill) {
                    (Some(mcp_server_name), Some(routing_skill)) => {
                    ...
                    (None, None) => AgentPluginCreateTarget::Complete { output },
                    _ => unreachable!("clap requires plugin integration options together"),
                };
                let report = create_agent_plugin(&AgentPluginCreateRequest {
                    config,
                    database,
                    target,
                    codegraph,
            ...
            PluginCommand::Uninstall {
                ...
                let report = uninstall_composed_integration(&AgentPluginUninstallRequest {
                    output,
                    mcp_server_name,
                    routing_skill,
                })?;
