...
use code_system_graph_model::{
    Edge, EdgeId, Evidence, EvidenceId, Node, NodeId, Provenance, RepoFreshnessState, RepoId
};
...
use crate::{
    ChangeImpactReport, ChangeRequest, ImpactReport, ImpactRequest, PullRequestInspection, SearchReport, SearchRequest, TraversalReport, TraversalRequest
};

mod contracts;

pub use contracts::{
    CONTRACT_NODE_KINDS, ContractAction, ContractCompatibility, ContractCompatibilitySummary, ContractDifference, ContractFinding, ContractIssue, ContractIssueSeverity, ContractLink, ContractReport, ContractRequest, ContractView, inspect_contracts
};
...
#[cfg(test)]
mod tests {
    use code_system_graph_model::{EdgeKind, EpistemicStatus, EvidenceId, NodeKind};

    use super::*;
    use crate::{CompatibilityFinding, CompatibilityReport, CompatibilityStatus};
    ...
    #[test]
    fn contract_list_should_not_validate_relationship_certainty() {
        let nodes = vec![
            node("node:a", NodeKind::HttpOperation, "a"),
            node("node:b", NodeKind::HttpOperation, "b"),
        ];
        let mut unresolved = edge("edge:1", "node:a", "node:b", &[]);
        unresolved.status = EpistemicStatus::Inferred;
        unresolved.confidence = 0.75;
    ...
    #[test]
    fn contract_validation_should_bound_issues_by_the_request_limit() {
        let nodes = (0..4)
            .map(|index| node(&format!("node:{index}"), NodeKind::HttpOperation, "http"))
            .collect::<Vec<_>>();
        let edges = (0..3)
            .map(|index| {
            ...
            .collect::<Vec<_>>();
        let mut input = request(ContractAction::Validate);
        input.limit = 2;
    ...
    #[test]
    fn contract_validation_prioritizes_errors_before_unknowns_when_truncated() {
        let nodes = vec![
            node("node:a", NodeKind::HttpOperation, "a"),
            node("node:b", NodeKind::HttpOperation, "b"),
        ];
        let mut unresolved = edge("edge:unknown", "node:a", "node:b", &[]);
        unresolved.status = EpistemicStatus::Inferred;
        let mut invalid = edge("edge:error", "node:a", "node:b", &[]);
        ...
        assert_eq!(report.issues[0].severity, ContractIssueSeverity::Error);
        assert_eq!(report.issues[0].entity_id.as_deref(), Some("edge:error"));
    }

    #[test]
    fn scoped_contract_validation_keeps_invalid_incident_evidence() {
        let nodes = vec![
            node("node:a", NodeKind::HttpOperation, "a"),
            node("node:b", NodeKind::HttpOperation, "b"),
            node("node:c", NodeKind::HttpOperation, "c"),
        ];
        let edges = vec![
            edge("edge:related", "node:a", "node:b", &["evidence:related"]),
            edge(
                "edge:unrelated",
                "node:b",
                "node:c",
        ...
        let mut related_evidence = evidence("evidence:related", "src/a.rs");
        related_evidence.confidence = 2.0;
        let mut unrelated_evidence = evidence("evidence:unrelated", "src/c.rs");
        unrelated_evidence.confidence = 2.0;
        let mut input = request(ContractAction::Validate);
        ...
        let report = report_or_panic(inspect_contracts(
            &nodes,
            &edges,
            &[related_evidence, unrelated_evidence],
            &[],
            &input,
        ));

        assert!(report.issues.iter().any(|issue| {
            issue.code == "contract.invalid_evidence_confidence"
                && issue.entity_id.as_deref() == Some("evidence:related")
        }));
        assert!(
            report
                .issues
                .iter()
                .all(|issue| issue.entity_id.as_deref() != Some("evidence:unrelated"))
        );
        assert_eq!(report.valid, Some(false));
    }

    #[test]
    fn contract_show_should_omit_issues_from_unrelated_contracts() {
        let nodes = vec![
            node("node:a", NodeKind::HttpOperation, "a"),
            node("node:b", NodeKind::HttpOperation, "b"),
            node("node:c", NodeKind::HttpOperation, "c"),
        ];
        let mut related = edge("edge:related", "node:a", "node:b", &[]);
        related.status = EpistemicStatus::Inferred;
        let mut unrelated = edge("edge:unrelated", "node:b", "node:c", &[]);
        unrelated.status = EpistemicStatus::Inferred;
        ...
        assert!(
            report
                .issues
                .iter()
                .any(|issue| issue.entity_id.as_deref() == Some("edge:related"))
        );
        assert!(
            report
                .issues
                .iter()
                .all(|issue| issue.entity_id.as_deref() != Some("edge:unrelated"))
        );
    }
