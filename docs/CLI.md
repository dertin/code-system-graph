...
executable fingerprint, and build-time source commit/dirty state when available. Receipts and
bindings generated by 1.0.x are rejected. Existing-plugin mode
...
```text
csgraph query <question> --config <global-manifest> --workspace <name> --database <db>
