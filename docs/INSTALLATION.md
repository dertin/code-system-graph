...
Code System Graph `1.0.3` is published on [crates.io](https://crates.io/crates/code-system-graph)
...
tar -xzf code-system-graph-x86_64-unknown-linux-gnu-v1.0.3.tgz
PREFIX="$HOME/.local" ./code-system-graph-x86_64-unknown-linux-gnu-v1.0.3/install.sh
```
...
`code-system-graph-x86_64-pc-windows-msvc-v1.0.3.zip`, and add its `bin` directory containing
...
```bash
PREFIX="$HOME/.local" ./code-system-graph-x86_64-unknown-linux-gnu-v1.0.3/uninstall.sh
```
