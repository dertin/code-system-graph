...
mkdir -p "$BASE_PLUGIN/.codex-plugin" "$BASE_PLUGIN/skills/unrelated-skill"
...
printf '%s\n' '---' 'name: unrelated-skill' 'description: Unrelated release smoke skill.' '---' \
  > "$BASE_PLUGIN/skills/unrelated-skill/SKILL.md"
"$PREFIX/bin/csgraph" plugin create \
  --output "$BASE_PLUGIN" \
...
[[ -f "$BASE_PLUGIN/skills/release-system-graph/SKILL.md" ]]
[[ -f "$BASE_PLUGIN/skills/unrelated-skill/SKILL.md" ]]
